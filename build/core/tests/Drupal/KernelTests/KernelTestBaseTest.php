...
class KernelTestBaseTest extends KernelTestBase {
  ...
  /**
   * Tests that drupal_generate_test_ua() can be called from within a test.
   */
  public function testGenerateTestUa() {
  ...
  public function testOutboundHttpRequest() {
    // When outbound HTTP requests are performed in tests
    // TestHttpClientMiddleware ends up calling drupal_generate_test_ua(), so
    // acts as a regression test.
    try {
      $this->container->get('http_client')->get('http://example.com');
    }
    catch (GuzzleException $e) {
      // The bug this test is testing caused a notice, actually completing the
      // HTTP request should not be required.
    }
  }
  ...
  /**
  * Ensures getExtensionsForModules() obeys 'file_scan_ignore_directories' setting.
  */
  public function testGetExtensionsForModules() {
    // Set directories to be ignored during filesystem scanning.
    $this->setSetting('file_scan_ignore_directories', ['user']);

    $this->expectException(\PHPUnit\Framework\Exception::class);
    $this->expectExceptionMessageRegExp('/Unavailable module: \'user\'/');

    $reflector = new \ReflectionObject($this);
    $method = $reflector->getMethod('getExtensionsForModules');
